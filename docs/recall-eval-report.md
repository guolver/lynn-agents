...
- 复现：`DATABASE_URL=... python scripts/eval_recall.py --report /tmp/recall-eval-report.md`（数据自动灌入与清理；勿直接覆盖本文件——脚本只生成表格，本文说明段为手写）。

**结论**：在同义改写场景下，关键词召回 Recall@5 为 0.000，向量召回为 1.000；向量检索在非关键词场景带来从完全不可用到满召回的提升。全体样本上向量召回 Recall@5 1.000 vs 关键词 0.469。

**局限性**：评测集规模较小（30 职位 / 8 候选人、每人 1–4 个相关职位），域内难负样本有限，向量召回在此设置下触及天花板（全项 1.000）；更大规模、更密集的同域职位下预期会出现梯度而非满分。结论的可靠部分是**相对差距的方向与量级**（改写场景 0 → 满召回），而非绝对数值。
